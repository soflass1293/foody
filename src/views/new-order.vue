  ...
  <div class="row w-100">
    ...
    <div class="col-xs-12 col-sm-12 col-xl-6 d-flex flex-column gap-2">
      <div class="card">
        <div class="card-body">
          <div class="row p-2">
            <div class="custom-title col-12 mb-2">Delivery Details</div>
            <div class="row">
              <div class="col d-flex flex-column">
                <label>Order Item</label>
                <value v-for="item in this.formData.items" :key="item">{{
                  item.value
                }}</value>
              </div>
      ...
      <div class="card">
        <div class="card-body">
          <div class="row p-2">
            <div class="col custom-title">Total Amount</div>
            <div class="col"></div>
  ...
  data() {
    ...
    return {
      orderItems: [
        ...
        {
          label: "Special Hamburger ( $13 ) ",
          value: "Special Hamburger",
          price: 13,
        },
        ...
        {
          label: "Kıymalı Kaşarlı Pide ( $22 )",
          value: "Kıymalı Kaşarlı Pide",
          price: 22,
        },
  ...
  methods: {
    submitHandler() {
      const data = {
        ...
        totalValue: this.formData?.items.reduce(
          (a, b) => +a + +b.price * b.quantity,
          0
        ),
        type: "New",
    ...
    minusQuantity(item) {
      const selectedItem = this.formData.items.find(
        (x) => x.value === item?.value
      );
      const selectedIndex = this.formData.items.findIndex(
        (x) => x.value === item?.value
      );
    ...
    changeDetect() {
      ...
      for (var i = 0; i < this.values.length; i++) {
        const selectedItem = this.orderItems.find(
          (x) => x.value === this.values[i]
        );
        list.push({
          value: selectedItem.value,
          name: selectedItem.label,
          price: selectedItem.price,
          quantity: this.formData.items.find((x) => x.value === this.values[i])
            ?.quantity
            ? this.formData.items.find((x) => x.value === this.values[i])
                ?.quantity
            : 0,
        });
      }
    ...
    checkInvalid() {
      const checkAnyItem =
        this.formData?.items?.findIndex((x) => x.quantity >= 1) !== -1;
      return !(
        checkAnyItem > 0 && this.$refs.form1?.node?.context?.state?.valid
      );
    },
  },
};
